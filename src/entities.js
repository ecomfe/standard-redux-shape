...
export const createTableUpdateReducer = (nextReducer = s => s) => (state = {}, action) => {
    ...
    const merging = reduce(entities, (chain, value, key) => chain.merge(key, value), immutable(entitySet));
    const [mergedEntitySet, diff] = merging.withDiff();
    const newState =  isEmpty(diff) ? state : {...state, [entitySetName]: mergedEntitySet};
